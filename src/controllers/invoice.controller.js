...
const createInvoice = async (req, res) => {
    const { invoice_number, tenant_id, lease_id, amount, due_date, status, type, paid_amount, payment_method, payment_reference, late_fee, reminder_status } = req.body;
    ...
    try {
        const [result] = await pool.query(
            'INSERT INTO invoices (invoice_number, tenant_id, lease_id, amount, due_date, status, paid_date, type, paid_amount, payment_method, payment_reference, late_fee, reminder_status, added_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [invoice_number, tenant_id, lease_id, amount, due_date, status, paid_date, invoiceType, pAmt, payment_method, payment_reference, late_fee || 0, reminder_status || 'None', req.user.id]
        );
...
const updateInvoice = async (req, res) => {
    const { id } = req.params;
    const { status, paid_date, paid_amount, payment_method, payment_reference, late_fee, reminder_status } = req.body;
    try {
        // Get old data for comparison
        const [oldInv] = await pool.query('SELECT status, invoice_number, tenant_id FROM invoices WHERE id = ?', [id]);

        let query = 'UPDATE invoices SET status=?, paid_date=?, paid_amount=?, payment_method=?, payment_reference=?, late_fee=?, reminder_status=? WHERE id=?';
        let params = [status, paid_date, paid_amount, payment_method, payment_reference, late_fee || 0, reminder_status || 'None', id];
        if (req.user.role === 'manager') {
            query += ' AND added_by=?';
            params.push(req.user.id);
        }
