...
// @desc    Get Phase 1 KPIs
// @route   GET /api/dashboard/kpis
const getKPIs = async (req, res) => {
    const { id: userId, role } = req.user;
    try {
        const addedBy = role === 'manager' ? 'AND added_by = ?' : '';
        const params = role === 'manager' ? [userId] : [];

        // 1. Active Leases
        const [activeLeases] = await pool.query(`SELECT COUNT(*) as total FROM leases WHERE status = 'active' ${addedBy}`, params);
        
        // 2. Expiring Leases (within 60 days)
        const [expiringLeases] = await pool.query(`SELECT COUNT(*) as total FROM leases WHERE status = 'active' AND end_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) ${addedBy}`, params);
        
        // 3. Open Maintenance Tickets
        const [openMaintenance] = await pool.query(`SELECT COUNT(*) as total FROM maintenance WHERE status IN ('pending', 'in_progress') ${addedBy}`, params);
        
        // 4. Overdue Invoices
        const [overdueInvoices] = await pool.query(`SELECT COUNT(*) as total FROM invoices WHERE status = 'Overdue' ${addedBy}`, params);

        res.json({
            success: true,
            data: {
                activeLeases: activeLeases[0].total,
                expiringLeases: expiringLeases[0].total,
                openMaintenance: openMaintenance[0].total,
                overdueInvoices: overdueInvoices[0].total,
                occupancyRate: 85 // Mocked for phase 1
            }
        });
    } catch (error) {
        console.error(error);
        res.status(500).json({ success: false, message: 'KPI error' });
    }
};
